...
export default function AdminSystemPage() {
  ...
  const handleExport = async (type) => {
    ...
    try {
      ...
      a.href = url;
      a.download = `mdx_billing_${type}_export_${new Date().getTime()}.json`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      
      showToast(`Successfully exported ${type} data.`);
    } catch (err) {
  ...
  const renderExport = () => (
    <div style={{ animation: "fadeIn 0.3s ease" }}>
      <CardHeader 
        ...
        description="Download raw JSON backups of the database. Useful for cold storage."
        color="#3B82F6"
      />

      <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
        {[
          { id: "full", name: "Full Database Backup", desc: "Exports all tables including users, shops, products, and sales.", icon: Database },
  ...
  const renderImport = () => (
    <div style={{ animation: "fadeIn 0.3s ease" }}>
      <CardHeader 
        ...
        description="Restore data from a JSON backup. Warning: Proceed with extreme caution."
        color="#8B5CF6"
      />
      ...
      <form onSubmit={handleImport} style={{
        ...
        <h3 style={{ margin: "0 0 8px", fontSize: "18px", fontWeight: 600, color: "var(--text)" }}>Select JSON Backup File</h3>
        <p style={{ margin: "0 0 24px", fontSize: "14px", color: "var(--text-dim)" }}>
          Must be a valid .json file generated by the MDX Billing export tool.
        </p>
