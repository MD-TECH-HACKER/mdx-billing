...
export async function POST(request) {
  try {
    ...
    const [products, today, month, previousMonth, customers, suppliers] = await sql.transaction([
      sql`SELECT product_id, title, cost_price, selling_price, stock, stock_base_unit, sold_base_unit,
      ...
      sql`SELECT COALESCE(SUM(total_amount), 0) AS revenue, COALESCE(SUM(total_profit), 0) AS profit,
      ...
      sql`SELECT COALESCE(SUM(total_amount), 0) AS revenue, COALESCE(SUM(total_profit), 0) AS profit,
          ...
          COALESCE((SELECT SUM(amount) FROM expenses WHERE shop_id = ${context.shopId}
            AND expense_date >= date_trunc('month', CURRENT_DATE)::date), 0) AS expenses
        FROM sales WHERE shop_id = ${context.shopId}
          AND (sale_status IS NULL OR sale_status = 'completed')
          AND created_at >= date_trunc('month', CURRENT_DATE)`,
      sql`SELECT COALESCE(SUM(total_profit), 0) AS profit
        FROM sales WHERE shop_id = ${context.shopId}
          AND (sale_status IS NULL OR sale_status = 'completed')
