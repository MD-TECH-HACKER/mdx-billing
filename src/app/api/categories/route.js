  ...
  try {
    ...
    const categories = await sql`
      SELECT c.*,
        ...
        COALESCE(SUM(CASE WHEN COALESCE(p.stock_base_unit, p.stock) <= COALESCE(p.low_stock_base_unit, p.reorder_level, 5) THEN 1 ELSE 0 END), 0) AS low_stock_count
      FROM categories c
...
export async function POST(request) {
  try {
    ...
    const rows = await sql`SELECT * FROM categories WHERE category_id = ${id}`;
